           ...
           % Get power flow
            ...
            % ### Call state equation: dx/dt = f(x,u)
                ...
                if Controll_type ==1
                    ...
                    % e_q = v_qr;
                    w = w0;
                    e_d = v_d -  Rf_ac*i_d + w*Lf_ac*i_q;
                    e_q = v_q -  Rf_ac*i_q - w*Lf_ac*i_d;
                    dv_d_i=0;
                    dv_q_i=0;
                    di_d_i=0;
                    di_q_i=0;
                ...
                dv_dc  = ((Pr-p)/v_dc)/C_dc;

                % Lf equation
                % e_d - v_od = -(di_ld/dt*Lf + Rf*i_ld - w*Lf*i_lq)
                % e_q - v_oq = -(di_lq/dt*Lf + Rf*i_lq + w*Lf*i_ld)
                di_d = (v_d - e_d - Rf_ac*i_d + w*Lf_ac*i_q)/Lf_ac;
                di_q = (v_q - e_q - Rf_ac*i_q - w*Lf_ac*i_d)/Lf_ac;
            	...
            	f_xu = [di_d; di_q; dv_dc; di; dtheta];
                Output = f_xu;
